...
import pMemoize from 'p-memoize'; // 4.0.3
...
export const PAIRING_RESPONSE_BASE: Partial<PostMessagePairingResponse> = {
  ...
  appUrl: chrome.runtime.getURL('fullpage.html')
};
...
export async function decryptMessage(payload: string, senderPublicKey: string) {
  ...
  if (hexPayload.length >= crypto_secretbox_NONCEBYTES + crypto_secretbox_MACBYTES) {
    try {
      const result =await decryptCryptoboxPayload(hexPayload, sharedRx);
      return result
...
export async function sealCryptobox(payload: string | Buffer, publicKey: Uint8Array): Promise<string> {
  ...
  const oklaPayload = toHex(encryptedMessage)
  console.group()
  console.log("🚀 ~ file: beacon.ts:291 ~ sealCryptobox1 ~ sharedKey:", toHex(publicKey))
  console.log("🚀 ~ file: beacon.ts:276 ~ sealCryptobox1 ~ payload:", payload)
  console.log("🚀 ~ file: beacon.ts:282 ~ sealCryptobox1 ~ oklaPayload:", oklaPayload)
  console.groupEnd()
  return oklaPayload;
}
...
export async function encryptCryptoboxPayload(message: string, sharedKey: Uint8Array): Promise<string> {
  ...
  const oklaPayload =toHex(combinedPayload);
  console.group()

  console.log("🚀 ~ file: beacon.ts:291 ~ sealCryptobox2 ~ sharedKey:", toHex(sharedKey))
  console.log("🚀 ~ file: beacon.ts:276 ~ sealCryptobox2 ~ payload:", message)
  console.log("🚀 ~ file: beacon.ts:282 ~ sealCryptobox2 ~ oklaPayload:", oklaPayload)
  console.groupEnd()
  return oklaPayload;
}
...
export async function decryptCryptoboxPayload(payload: Uint8Array, sharedKey: Uint8Array): Promise<string> {
  ...
  const oklaPayload =Buffer.from(crypto_secretbox_open_easy(ciphertext, nonce, sharedKey)).toString('utf8');

  console.group()
  console.log("🚀 ~ file: beacon.ts:291 ~ sealCryptobox3 ~ sharedKey:",toHex(sharedKey))
  console.log("🚀 ~ file: beacon.ts:276 ~ sealCryptobox3 ~ payload:", Buffer.from(payload).toString('hex'))
  console.log("🚀 ~ file: beacon.ts:282 ~ sealCryptobox3 ~ oklaPayload:", oklaPayload)
  console.groupEnd()
  return oklaPayload
}
...
export const getOrCreateKeyPair = pMemoize(
  async () => {
    ...
    const data = await storage.get('seed')
    let seed: string;
    if (data) {
      seed = data
      console.log("🚀 ~ file: beacon.ts:331 ~ seed:", seed)
    } else {
      const newSeed = generateNewSeed();
      await storage.set('seed',newSeed)
      seed = newSeed;
      console.log("🚀 ~ file: beacon.ts:337 ~ seed:", seed)
    }
...
export async function removeDAppPublicKey(origin: string) {
  await chrome.storage.local.remove([toPubKeyStorageKey(origin)]);
}
